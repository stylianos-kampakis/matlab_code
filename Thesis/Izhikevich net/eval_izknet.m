function [result action_series voltage]=eval_izknet(net,input)
    ...
    for i=1:numel(weights)
        %Set the action_matrix to zero
          ...
          for j=1:numel(v{i})
              ...
              x=v{i}(j)+net.k.*(0.04.*v{i}(j).^2+5.*v{i}(j)+140-u{i}(j)+I(j)); 
            ...
            %
              if ((x<30) && ((v{i}(j))~=30))
                 ...
                 v{i}(j)=v{i}(j)+net.k.*(0.04.*v{i}(j).^2+5.*v{i}(j)+140-u{i}(j)+I(j)); ; 
                 u{i}(j)=u{i}(j)+a{i}(j).*(b{i}(j).*v{i}(j)-u{i}(j));  
...
found=0;
     ...
     for i=1:ms
         if found==0
             for j=1:net.number_of_output_neurons
                    if action_series{net.output_layer,i}(j)==1
