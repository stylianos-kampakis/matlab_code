function [net training_success]=pure_train_izknet3(net,in,out,options)
upper_count=3;
lower_count=50;
...
for i=1:size(in,2)
  % disp(['This is input' num2str(i)])
   ...
   action_series_outputs=derive_action(action_series(first_index+1:second_index,:),1);
...
if ~(sum(result==out(:,i))==numel(result))    
   ...
   for j=1:numel(result)   
       ...
       if(result(j)~=0 && out(j,i)==0)
            Discrepancy_inhibition=derive_action(action_series(first_index+1:second_index,:),1);
          
                Discrepancy_inhibition(Discrepancy_inhibition==0)=-1;
tic
            while (action_series_outputs(j)<=Discrepancy_inhibition(j)) && action_series_outputs(j)~=0
       ...
       if(result(j)==0 && out(j,i)==1)
           tic
       while action_series_outputs(j)==0
             ...
             pos_or_neg=sum(net.layer_weights{1}(j,:).*action_series_inputs);
              ...
              %come later can have an inhibitory effect!
                ...
                % action_series_inputs(find(net.layer_weights{1}(j,:)>=0))=0;
                 net.layer_weights{1}(j,:)=net.layer_weights{1}(j,:)-Excitatory_learning_rate*...
                 Excitatory_function(abs(net.layer_weights{1}(j,:))).*action_series_inputs-Excitatory_momentum.*action_series_inputs;                              
                     end
      ...
      if toc>5
       ...
       if(result(j)~=1 && out(j,i)==1)
         Discrepancy_excitation=derive_action(action_series(first_index+1:second_index,:),1);
         
         Discrepancy_excitation(Discrepancy_excitation==0)=net.ms; 
tic
         while(action_series_outputs(j)>=Discrepancy_excitation(j))
