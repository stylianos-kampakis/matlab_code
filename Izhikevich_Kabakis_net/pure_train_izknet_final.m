function [net success training_success time]=pure_train_izknet4(net,in,out,options)
tStart=tic;
%Create the default optionset if one is not already in the arguments
if(nargin<4)
    options=pure_train_optionset_final;
end
...
for i=1:size(in,2)
  ...
  % action_series=clean_action_series(action_series,sum(net.sensor_neurons_distribution),numel(result)); 
   ...
   action_series_inputs=sum(action_series(1:first_index,:),2);    
   action_series_outputs=sum(action_series(first_index+1:second_index,:),2);

   action_series_inputs=action_series_inputs';
   action_series_outputs=action_series_outputs';
...
%Permute the output neurons once again
 ...
 %If the output neuron should have fired but it did not
       if(action_series_outputs(j)~=1 && out(j,i)==1)
                ...
                while(action_series_outputs(j)<1)
